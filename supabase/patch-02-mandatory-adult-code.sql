-- ===========================================================================
-- KINDLY patch 02 — the grown-up code is mandatory
-- ===========================================================================
-- Run this in the Supabase SQL Editor on the project you already applied the
-- schema and patch 01 to. It replaces four functions and adds one; it creates
-- no tables, drops nothing, and touches no rows.
--
-- What it fixes, in order of seriousness:
--
--   1. verify_caregiver_pin() FAILED OPEN. With no row in caregiver_pins it
--      returned ok=true for any code at all — a screen that looked like a lock
--      and was not one. It now answers 'not_configured' honestly.
--   2. bootstrap_family() accepted a null code, so a family space could be
--      created with no code for the adult check to check against.
--   3. set_adult_verification_mode() accepted 'none', which switched the adult
--      check off entirely.
--   4. There was no way for the app to ask whether a code exists, because
--      caregiver_pins has no client policy by design. get_adult_verification()
--      returns that one fact and never the hash.
--
-- After running it: every new family must choose a code, and the code cannot
-- be switched off. Families created before this patch keep working — the adult
-- check now tells them plainly that no code is set and sends them to Settings
-- to choose one, instead of asking for a code that never existed.
-- ===========================================================================

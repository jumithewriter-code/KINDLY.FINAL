-- ===========================================================================
-- KINDLY patch 03 — the operator dashboard
-- ===========================================================================
-- Run this in the Supabase SQL Editor on the project that already has the
-- schema plus patches 01 and 02.
--
-- It adds one table (kindly.operators) and two functions. It drops nothing,
-- changes no existing table, and touches no rows.
--
-- IMPORTANT — this grants nobody anything on its own. After running it, make
-- yourself an operator by hand:
--
--     insert into kindly.operators (user_id, note)
--     select id, 'me' from auth.users where email = 'you@example.com';
--
-- That INSERT is the only way in. kindly.operators has RLS forced and no policy
-- at all, so no application code can read it, discover who is on it, or add to
-- it. A caregiver cannot promote themselves.
--
-- What an operator can then see: counts and durations across every family.
-- What they still cannot see: any name, any message, or any identifier that
-- could be joined back to a family. That is a property of the function's return
-- shape, not of the page that renders it.
-- ===========================================================================
